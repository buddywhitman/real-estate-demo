       ...
       <div className="flex justify-between items-center">
         ...
         <div className="flex gap-4 items-center">
             <div className="relative group">
                 <button className="flex items-center gap-2 bg-white/5 border border-glass-border px-3 py-1.5 rounded-lg text-xs text-white hover:bg-white/10">
                 ...
                 <div className="absolute right-0 top-full mt-2 w-32 bg-[#151515] border border-glass-border rounded-lg shadow-xl hidden group-hover:block z-20">
                     <button onClick={() => setTimeRange('7 Days')} className="w-full text-left px-3 py-2 text-xs text-gray-300 hover:bg-white/10">Last 7 Days</button>
                     <button onClick={() => setTimeRange('30 Days')} className="w-full text-left px-3 py-2 text-xs text-gray-300 hover:bg-white/10">Last 30 Days</button>
                     <button onClick={() => setTimeRange('Year')} className="w-full text-left px-3 py-2 text-xs text-gray-300 hover:bg-white/10">This Year</button>
                 </div>
             </div>
       ...
       <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
           <GlassCard className="flex flex-col justify-between" noPadding>
               <div className="p-4">
                   ...
                   <h3 className="text-2xl font-bold text-white mt-1">14.2%</h3>
                   <span className="text-xs text-green-400 flex items-center mt-2"><TrendingUp size={10} className="mr-1"/> +2.4% vs last week</span>
               </div>
           ...
           <GlassCard className="flex flex-col justify-between" noPadding>
               <div className="p-4">
                   ...
                   <h3 className="text-2xl font-bold text-white mt-1">2m 4s</h3>
           ...
           <GlassCard className="flex flex-col justify-between" noPadding>
               <div className="p-4">
                   ...
                   <h3 className="text-2xl font-bold text-white mt-1">₹ 42 Cr</h3>
           ...
           <GlassCard className="flex flex-col justify-between" noPadding>
               <div className="p-4">
                   ...
                   <span className="text-xs text-red-400 flex items-center mt-2"><Users size={10} className="mr-1"/> 12 Hot Leads</span>
               </div>
                ...
                {data.map((d, i) => (
                   <div key={i} className="flex-1 flex flex-col justify-end items-center gap-1 h-full group">
                      <div className="w-full flex gap-1 items-end justify-center h-full">
                          <div className="w-1/2 bg-brand-500/20 hover:bg-brand-500/40 transition-colors rounded-t-sm relative" style={{height: `${d.leads * 5}%`}}>
                             <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-black text-white text-[10px] px-1 rounded opacity-0 group-hover:opacity-100">{d.leads}</div>
                          </div>
                          <div className="w-1/2 bg-purple-500/20 hover:bg-purple-500/40 transition-colors rounded-t-sm relative" style={{height: `${d.visits * 10}%`}}>
                             <div className="absolute -top-6 left-1/2 -translate-x-1/2 bg-black text-white text-[10px] px-1 rounded opacity-0 group-hover:opacity-100">{d.visits}</div>
                          </div>
                      </div>
                      {/* X-Axis Label */}
                      <span className="text-[10px] text-gray-500 -mb-6">{d.label}</span>
                   </div>
                ))}
             </div>
